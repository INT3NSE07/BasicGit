...
appName = 'basicgit'
basicGitVersion = '1.0.12'

# Metadata being stored for each entry in the git index
...
class ObjectType(enum.Enum):
    ...
    Tree: This object stores directories present in our project.
    Blob: This object stores the original content.
    """
    commit = 1
    tree = 2
    blob = 3
...
def read_file(path):
    """Read contents of file at a given path as bytes."""
    with open(path, 'rb') as f:
        return f.read()


def write_file(path, data):
    """Write data to file at given path as bytes."""
    with open(path, 'wb') as f:
        f.write(data)


def init(directory):
    """Create directory for repository and initialize folders and files in .git directory."""
    gitPath = os.path.join(directory, '.git')

    Path(gitPath).mkdir(parents=True, exist_ok=True)
    for folderName in ['objects', 'refs', 'refs/heads']:
        Path(os.path.join(gitPath, folderName)).mkdir(
            parents=True, exist_ok=True)

    write_file(os.path.join(gitPath, 'HEAD'), b'ref: refs/heads/master')
    print(f'Initialized empty Git repository in {directory}')


def hash_object(data, obj_type, write=True):
    """Compute hash of data and write to the object database if write is True.
    Return the SHA-1 of the object.
    """
    header = f'{obj_type} {len(data)}'.encode()
    data_with_header = header + b'\x00' + data
    sha1 = hashlib.sha1(data_with_header).hexdigest()
    if write:
        path = os.path.join('.git', 'objects', sha1[:2], sha1[2:])
        if not os.path.exists(path):
            Path(os.path.dirname(path)).mkdir(exist_ok=True)
            write_file(path, zlib.compress(data_with_header))
    return sha1


def find_object(sha1_prefix):
    """Find object with the given SHA-1 prefix and return its path in the object database"""
...
def read_object(sha1_prefix):
    """Read object with given the SHA-1 prefix and return a tuple of (object_type, data_bytes)"""
...
if __name__ == '__main__':
    ...
    sub_parser = sub_parsers.add_parser('version',
                                        help=f'prints the {appName} version')

    sub_parser = sub_parsers.add_parser('init',
                                        help=f'create an empty {appName} repository')
    sub_parser.add_argument('directory',
                            help=f'directory name for the new {appName} repository')
    ...
    if args.command == 'version':
        print(f"{appName} version {basicGitVersion}")
